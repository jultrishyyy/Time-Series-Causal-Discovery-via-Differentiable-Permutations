"""
Command-line interface for cMLP Neural Granger Causality.

Reads a CSV file, trains a cMLP model using GISTA, and outputs the 
...
from cmlp import cMLP, train_model_gista

def run_causal_discovery(data: np.ndarray, args: argparse.Namespace) -> np.ndarray:
    """
    Trains the cMLP model and extracts the Granger causality matrix.
    """
    ...
    # Data is expected as a single batch (batch, T, p)
    ...
    # Train model
    print("Starting cMLP training with GISTA...")
    train_model_gista(
        model,
        data_tensor,
...
def main():
    parser = argparse.ArgumentParser(
        description='Discover causal relationships in time series using cMLP Neural Granger Causality.',
    ...
    parser.add_argument('--hidden_layers', type=int, nargs='+', default=[10], help='List of hidden units per layer.')
    parser.add_argument('--penalty', type=str, default='H', choices=['GL', 'GSGL', 'H'], help='Sparsity penalty: Group Lasso (GL), Group Sparse Group Lasso (GSGL), or Hierarchical (H).')
    parser.add_argument('--max_iter', type=int, default=1000, help='Maximum number of training iterations.')
    parser.add_argument('--check_every', type=int, default=100, help='Frequency of progress checks.')
    parser.add_argument('--learning_rate', type=float, default=0.01, help='Learning rate for the optimizer.')
    parser.add_argument('--lambda_param', type=float, default=0.1, help='Lambda for the group sparsity penalty.')
    parser.add_argument('--lambda_ridge', type=float, default=0.01, help='Lambda for the ridge penalty on hidden layers.')
